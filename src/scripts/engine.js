...
async function createCardImage(cardId, fieldSide) {
  ...
  if (fieldSide === state.playerSides.player1) {
    cardImage.addEventListener("mouseover", () => drawSelectCards(cardId));
...
async function setCardsField(cardId) {
  ...
  setTimeout(() => milleniumImg.classList.remove("pulse"), 600);
}
...
async function ShowHiddenCardFieldsImages(vaule) {
  if (vaule === true) {
...
async function hidedenCardDetails() {
...
async function updateScore() {
  state.score.scoreBox.innerHTML = `Vitórias: ${state.score.playerScore}  Derrotas: ${state.score.computerScore}`;
}
...
document.getElementById("start-button").addEventListener("click", () => {
  ...
  if (bgm) {
    try {
      bgm.play(); // Agora o navegador permite
